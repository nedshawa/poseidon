...
```bash
cp -r skills/[skill-name]/ History/Backups/[skill-name]-backup-$(date +%Y%m%d)/
```

**Note:** Backups go to `History/Backups/`, NEVER inside skill directories.
...
find skills/[SkillName]/ -type d -mindepth 2 -maxdepth 3
```
