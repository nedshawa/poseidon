...
const entityIndex = JSON.parse(
  fs.readFileSync('skills/parser/entity-index.json', 'utf-8')
);
...
```typescript
import { scrapeAsMarkdown } from 'skills/mcp/Providers/brightdata/actors';
...
// Main processing
const entityIndexPath = 'skills/parser/entity-index.json';
...
const schema = JSON.parse(
  fs.readFileSync('skills/parser/schema/content-schema.json', 'utf-8')
);
...
processedArticles.forEach((article: any, index: number) => {
  ...
  if (!valid) {
    ...
    const filename = `skills/parser/output/${timestamp}_batch-${index + 1}.json`;
...
```bash
# Check entity counts
cat skills/parser/entity-index.json | jq '{
