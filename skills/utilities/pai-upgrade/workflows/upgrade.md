...
- Read memory/STATE/current-work.json
...
1. Load channel config:
   bun docs/Tools/LoadSkillConfig.ts ../youtube-channels.json
...
4. For NEW videos, extract transcripts:
   bun docs/Tools/GetTranscript.ts '<video-url>'
...
1. Look in docs/USER/SKILLCUSTOMIZATIONS/PAIUpgrade/
...
1. Load the github_trending config from user-sources.json:
   Read docs/USER/SKILLCUSTOMIZATIONS/PAIUpgrade/user-sources.json
