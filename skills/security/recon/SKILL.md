...
`docs/USER/SKILLCUSTOMIZATIONS/Recon/`
...
- `naabu` - Port scanning
- Note: Requires security MCP profile (`MCPs/swap-mcp security`)
...
Reports saved to:
- **Work directory** (`memory/WORK/{current_work}/`) - For iterative artifacts during investigation
- **MEMORY/RESEARCH/** (`memory/RESEARCH/YYYY-MM/`) - For pentest engagements and formal assessments
...
Assistant: Activating recon skill with passive-recon workflow.
     ...
     Report: memory/WORK/{current_work}/recon-example-com/
```
...
**Security Skills:**
- `skills/Investigation/` - Entity and people reconnaissance (OSINT)
- `skills/Security/WebAssessment/` - Web application testing
