...
TELOS is {PRINCIPAL.NAME}'s life framework stored in `docs/USER/TELOS/`. It contains:
...
!`FILE="$1"; CONTENT="$2"; DESCRIPTION="$3"; bun commands/update-telos.ts "$FILE" "$CONTENT" "$DESCRIPTION"`
...
!`head -50 docs/USER/TELOS/updates.md`

## View Specific TELOS File
!`FILE="$1"; cat docs/USER/TELOS/"$FILE"`
...
Example:
```bash
bun commands/update-telos.ts "BOOKS.md" "- *Project Hail Mary* by Andy Weir" "Added favorite book: Project Hail Mary"
```
...
The script is at: `commands/update-telos.ts`

All backups are stored in: `docs/USER/TELOS/Backups/`

All changes are logged in: `docs/USER/TELOS/updates.md`
