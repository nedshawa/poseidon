...
1. Read `memory/STATE/current-work.json`
...
```bash
History/research/YYYY-MM-DD_description/
```
...
```
History/research/2025-10-26_podcast-analysis/
...
1. **Check if hooks captured the output:**
   ...
   ls -lt History/research/ | head -5

   # Verify your research directory exists
   ls History/research/YYYY-MM-DD_description/
   ```

2. **If hooks did NOT capture automatically:**
   ```bash
   # Create directory structure manually
   mkdir -p History/research/YYYY-MM-DD_description/
...
3. **Confirm all files saved:**
   ```bash
   ls -lah History/research/YYYY-MM-DD_description/
...
WORK_DIR=$(jq -r '.work_dir' memory/STATE/current-work.json)
...
mkdir -p History/research/$(date +%Y-%m-%d)_podcast-analysis/
...
ls -lah History/research/$(date +%Y-%m-%d)_podcast-analysis/
...
6. **Save to history** - Final outputs to `History/research/YYYY-MM-DD_description/`
