...
1. **Load Art preferences:**
   ```
   docs/USER/SKILLCUSTOMIZATIONS/Art/PREFERENCES.md
   ```
...
3. **Use Theme Constants:**
   ```
   skills/Media/Remotion/Tools/Theme.ts
   ```

4. **Reference images** (when visual style reference needed):
   ```
   skills/Media/Art/Examples/
   ```

## PAI Theme Quick Reference

```typescript
import { PAI_THEME } from 'skills/Media/Remotion/Tools/Theme'
...
```typescript
import { PAI_THEME, titleScreenStyle, fadeInterpolation } from 'skills/Media/Remotion/Tools/Theme'
