...
**Data access routes through the data source index.** When external data is needed, consult `data-sources.yaml` for the domain, check manifest for enabled sources, and use the highest-quality enabled source. If premium is disabled, follow the fallback chain (premium → standard → free). Never blind-attempt a data source without checking if it's enabled. The system-reminder shows available sources per domain — if a domain shows "none enabled," tell the user what needs to be configured. Founding Principle #22.
