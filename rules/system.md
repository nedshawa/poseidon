...
**Skills must carry complete metadata.** Every skill in skill-index.yaml must have all 18 metadata fields populated: name, tier, priority, requires, description, category, tags, version, author, status, created, updated, requires_services, complexity, sub_skills, workflows, has_tools. Incomplete metadata means the skill cannot be properly discovered, filtered, or managed. When creating or acquiring a skill, the skill-builder MUST populate all fields. Founding Principle #12.
...
**All behavior governed by the capability manifest (CRITICAL).** The manifest (`poseidon-manifest.yaml`) is the single source of truth for what this Poseidon instance can do. If a service is not enabled in the manifest, it MUST NOT be attempted, referenced, or assumed available. Research ONLY dispatches agents listed in `capabilities.research_agents`. Voice ONLY fires if `capabilities.voice_enabled` is true. Secret management ONLY uses the backend specified in `secrets.backend`. When a new API key is captured, the manifest MUST be updated to enable that service. Never attempt a service not in the manifest — it wastes time, context, and user trust. Founding Principle #21.

**Data access routes through the data source index.** When external data is needed, consult `data-sources.yaml` for the domain, check manifest for enabled sources, and use the highest-cost-tier enabled source. If paid is disabled, follow the fallback chain (paid → freemium → free). Never blind-attempt a data source without checking if it's enabled. The system-reminder shows available sources per domain — if a domain shows "none enabled," tell the user what needs to be configured. When a new API key is detected, auto-identify via key_patterns in data-sources.yaml — if no match, ask the user 5 classification questions and auto-index. data-sources.yaml is the SINGLE source of truth for all external services (services.yaml is deleted). Founding Principle #22.
